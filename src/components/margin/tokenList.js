...
class TokenList extends React.Component {
  ...
  componentDidMount() {
    // 计价币种
    const quoteTokenIds = this.props.config.marginQuoteToken
      .filter(
        (item) =>
          item && item.quoteTokenSymbols && item.quoteTokenSymbols.length > 0
      )
    ...
    let index = 1;
    let find = false;
    this.props.config.marginQuoteToken.forEach((item, i) => {
      // 计价币对
      (item.quoteTokenSymbols || []).forEach((it) => {
        if (
          it.allowMargin &&
          it.baseTokenId == token1 &&
          it.quoteTokenId == token2 &&
          !find
        ) {
          find = true;
          index = i + 1;
    ...
    this.setState({
      quoteTokens: ["CHOOSE", ...quoteTokenIds],
      quoteTokenIndex: /favorite/i.test(this.props.history.location.search)
        ? 0
        : index,
  ...
  async goto(exchangeId, baseToken, quoteToken) {
    ...
    let url = route_map.margin + "/" + baseToken + "/" + quoteToken;
  ...
  renderData(data, favorite) {
    const { classes, borrowableTokenMultiple } = this.props;
    if (!data.length) {
      return (
      ...
      return (
        ...
        >
          <div className={classes.fav}>
            ...
            />
            {d.baseTokenName}
            {this.state.quoteTokenIndex == 0 ? `/${d.quoteTokenName}` : ""}
            {/* 自选框不显示杠杆倍数 */}
            {this.state.quoteTokenIndex > 0 &&
            borrowableTokenMultiple[d.baseTokenName] &&
            borrowableTokenMultiple[d.baseTokenName].leverage ? (
              <span className={classes.leverage}>
                {borrowableTokenMultiple[d.baseTokenName].leverage}X
              </span>
            ) : (
              ""
            )}
  ...
  render() {
    ...
    const quoteToken = this.props.config.marginQuoteToken.filter(
      (item) =>
        item && item.quoteTokenSymbols && item.quoteTokenSymbols.length > 0
    );
    if (this.state.quoteTokenIndex == 0) {
      quoteToken.map((item) => {
    ...
    if (this.state.quoteTokenIndex > 0) {
      quoteToken.length &&
        (
          (quoteToken[this.state.quoteTokenIndex - 1] || {})[
            "quoteTokenSymbols"
          ] || []
        ).map((item) => {
          if (
        ...
        <div className={classes.token_title}>
          {this.state.quoteTokens.map((item, i) => {
            return (
              <span
                key={item}
                className={this.state.quoteTokenIndex == i ? "on" : ""}
