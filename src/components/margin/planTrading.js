...
class PlanTrading extends React.Component {
  constructor() {
    ...
    this.state = {
      ...
      order_side: "", // 下单方向
      buy_type: CONST.ORDER_TYPE.LIMIT, // 限价交易
      buy_trigger_price: "",
      sale_trigger_price: "",
  ...
  shouldComponentUpdate(preProps, preState) {
    ...
    if (
      ...
      loading["lever/createPlanOrder"] != nextloading["lever/createPlanOrder"]
    ) {
      return true;
    }
  ...
  handleChange(e) {
    ...
    if (n == "sale_trigger_price" || n == "buy_trigger_price") {
      ...
      if (n == "buy_quantity") {
        ...
        let progress = 0;
        let buyMax = this.props.buy_max;
        if (this.state.buy_type == CONST.ORDER_TYPE.MARKET) {
          buyMax = this.props.token2_quantity;
        }
        if (d && Number(buyMax) && this.props.buy_price) {
          progress = Math.min(
            100,
            math
              .chain(d)
  ...
  render() {
    ...
    const buy_trigger_price_rates = helper.currencyValue(
      this.props.rates,
      this.state.buy_trigger_price,
      this.props.token2,
      window.localStorage.lang,
    ...
    const sale_trigger_price_rates = helper.currencyValue(
      this.props.rates,
      this.state.sale_trigger_price,
      this.props.token2,
      window.localStorage.lang,
          ...
          <div className={classes.form} span={12}>
                  ...
                  this.state.buy_trigger_price_focus ? (
                    <p className={classes.startAdorn}>
                      ...
                      this.props.rates[this.props.token2]
                        ? `≈ ${buy_trigger_price_rates[1]}${buy_trigger_price_rates[2]}`
                        : ""}
                    </p>
                  ) : (
                    ""
            ...
            <div className={classes.planTradingBtn}>
              {this.props.userinfo.userId ? (
                this.props.loading.effects["lever/createPlanOrder"] &&
                ...
                startAdornment={
                  ...
                  this.state.sale_trigger_price_focus ? (
                    <p className={classes.startAdorn}>
                      {this.state.sale_trigger_price &&
                      this.props.rates[this.props.token2]
                        ? `≈ ${sale_trigger_price_rates[1]}${sale_trigger_price_rates[2]}`
                        : ""}
                    </p>
                  ) : (
                    ""
