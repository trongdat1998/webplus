...
class DataFeed {
  ...
  reset(
    ...
    // that.
    data.map((item) => {
      that.onSymbolResolvedCallback && that.onSymbolResolvedCallback(item);
    });
  }
  // 商品信息
  ...
  getBars(
    ...
    this.onHistoryCallback[
      symbolInfo.name == this.name ? "index" : "kline"
    ] = onHistoryCallback;
  ...
  async history(symbolInfo, from, to, _cb) {
    const key = symbolInfo.name == this.name ? "index" : "kline";
    const url =
      symbolInfo.name == this.name ? URLS.index_kline : URLS.kline_history;
  ...
  firstRequestCallback = (symbolInfo, from, to, cb) => {
    let t = 0;
    const loading_key = symbolInfo.name == this.name ? "index" : "kline";
    const data_key =
      symbolInfo.name == this.name ? "indexKline_source_f" : "kline_source_f";
