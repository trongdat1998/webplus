...
let datafeed;
let palette2 = window.palette2["Light"];
...
class Kline extends React.Component {
  ...
  resetKline = async (t) => {
    ...
    if (this.state.datafeed) {
      this.state.datafeed.reset(
        this.props.indexToken, // symbol_info.symbolName,
    ...
    if (this.state.tvwidget && this.state.tvwidget._ready) {
      ...
      this.state.tvwidget.chart().setSymbol(this.props.indexToken); // symbol_info.symbolName
      this.tradviewAction("chartReset")();
      this.tradviewAction("timeScaleReset")();
    }
    // 重新订阅
  ...
  start = () => {
    ...
    tvwidget = new TradingView.widget({
      ...
      autosize: true,
      symbol: this.props.indexToken, // symbol_info.symbolName,
    ...
    tvwidget.onChartReady(() => {
      ...
      // onAutoSaveNeeded 用户修改了图表
      tvwidget.subscribe("onAutoSaveNeeded", (id) => {
        const ids = tvwidget.chart().getAllStudies();
        if (ids.length == 0) {
          this.toggle();
        }
      });
  ...
  initStudy = async (tvwidget, show_contrace) => {
    ...
    const exchangeId = this.props.exchange_id;
    const symbol_info = this.props.config.symbols_obj.all[symbolId] || {};
    if (this.state.show_contrace || show_contrace) {
      const id = await tv
        .chart()
        .createStudy(
          "Compare",
  ...
  toggle = () => {
    if (this.state.show_contrace) {
      this.state.tvwidget.chart().removeEntity(this.state.show_contrace);
      this.setState({
        show_contrace: false,
      });
    } else {
      this.initStudy(null, true);
    }
  };
  render() {
    const { classes } = this.props;
        ...
        <Grid container className={classes.kline_btns} alignItems="center">
          ...
          >
            <div className={classes.chartype}>
              <span
                className={this.state.show_contrace ? "choose" : ""}
                onClick={this.toggle}
              >
