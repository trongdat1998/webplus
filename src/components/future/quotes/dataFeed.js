...
class DataFeed {
  constructor(
    ...
    this.loading = { kline: false, index: false };
  ...
  async readTicker() {
    ...
    const tickers_index = WSDATA.getData("indexKline_source");
    ...
    const g_k_ticker =
      tickers[
        "kline_" + this.exchange_id + this.symbol_id + this.getResolition()
      ];
    const g_k_ticker_index =
      tickers_index["indexKline_" + this.symbol_id + this.getResolition()];
    ...
    if (
      g_k_ticker &&
      params.indexOf(this.symbol_id) > -1 &&
      !this.loading.kline
    ) {
      for (let listenerGuid in this._subscribers) {
        const listener = this._subscribers[listenerGuid];
        if (
          listener.symbolInfo.name == this.name &&
          listener.resolution == this.resolution
        ) {
          let d = {
            time: g_k_ticker.t,
    ...
    if (g_k_ticker_index && !this.loading.index) {
      for (let listenerGuid in this._subscribers) {
        const listener = this._subscribers[listenerGuid];
        if (
          listener.symbolInfo.name == this.othersymbols[0] &&
          listener.resolution == this.resolution
        ) {
          let d = {
            time: g_k_ticker_index.t,
            open: Number(
              helper.digits(g_k_ticker_index.o, this.max_digits_number)
            ),
            high: Number(
              helper.digits(g_k_ticker_index.h, this.max_digits_number)
            ),
            low: Number(
              helper.digits(g_k_ticker_index.l, this.max_digits_number)
            ),
            close: Number(
              helper.digits(g_k_ticker_index.c, this.max_digits_number)
            ),
            volume: Number(g_k_ticker_index.v),
          };
          this.update(listenerGuid, d);
        }
      }
    }
    WSDATA.clear("indexKline_source");
  ...
  reset(
    ...
    that.symbol_id = symbol_id;
    that.othersymbols = othersymbols;
    // that.
  ...
  getBars(
    ...
    this.onHistoryCallback[
      symbolInfo.name == this.name ? "kline" : "index"
    ] = onHistoryCallback;
    ...
    return this.history(symbolInfo, from * 1000, to * 1000, onHistoryCallback);
  }
  ...
  async history(symbolInfo, from, to, _cb) {
    const key = symbolInfo.name == this.name ? "kline" : "index";
    const url =
      symbolInfo.name == this.name ? URLS.kline_history : URLS.index_kline;
    this.loading[key] = true;
    let interval = this.getResolition();
    let noData = false;
    const cb = _cb || this.onHistoryCallback[key];
    // 2010/01/01 00:00:00
    ...
    try {
      const result = await Requset(url, {
        body: {
          symbol:
            key == "kline"
              ? this.exchange_id + "." + this.symbol_id.toUpperCase()
              : this.othersymbols[0].toUpperCase(),
  ...
  // 第一次拉取历史记录callback
  firstRequestCallback = (symbolInfo, from, to, cb) => {
    let t = 0;
    const loading_key = symbolInfo.name == this.name ? "kline" : "index";
    const data_key =
      symbolInfo.name == this.name ? "kline_source_f" : "indexKline_source_f";
    const history = this.history;
    const getdata = () => {
      const datas = WSDATA.getData(data_key);
      const key =
        symbolInfo.name == this.name
          ? "kline_" + this.exchange_id + this.symbol_id + this.getResolition()
          : "indexKline_" + this.symbol_id + this.getResolition();
      ...
      if (data && data.length) {
        ...
        this.bar = bar;
        this.loading[loading_key] = false;
        WSDATA.clear(data_key, key);
      } else {
        t++;
        if (t > 10) {
          this.history(symbolInfo, from, to, cb);
          return;
        }
        setTimeout(() => {
          getdata();
